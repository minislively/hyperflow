...
function parseCurveSegments(path: string) {
  const match = path.match(
    /^M\s*(-?\d+(?:\.\d+)?)\s*(-?\d+(?:\.\d+)?)\s+C\s*(-?\d+(?:\.\d+)?)\s*(-?\d+(?:\.\d+)?),\s*(-?\d+(?:\.\d+)?)\s*(-?\d+(?:\.\d+)?),\s*(-?\d+(?:\.\d+)?)\s*(-?\d+(?:\.\d+)?)$/,
  );
  if (!match) throw new Error(`missing cubic segments in path: ${path}`);
  return {
    move: { x: Number(match[1]), y: Number(match[2]) },
    sourceControl: { x: Number(match[3]), y: Number(match[4]) },
    targetControl: { x: Number(match[5]), y: Number(match[6]) },
...
function expectHorizontalLaneCurve(path: string, maxLift = 40) {
...
async function dragNodeBurst(
  ...
  await page.mouse.move(box.x + box.width / 2 + offsetX, box.y + box.height / 2 + offsetY, { steps });
  await page.mouse.up();
}

async function runBenchmarkPerfBursts(page: import("@playwright/test").Page) {
...
test("react starter opens the editor-first surface at the locale root", async ({ page }) => {
  ...
  expect(
    await page.evaluate(() => Object.prototype.hasOwnProperty.call(window, "__HF_EDITOR_PERF_READOUT__")),
  ).toBeFalsy();
});
...
test("selected edge target endpoints enter reconnect-target on the first gesture", async ({ page }) => {
  ...
  await page.mouse.move(endpointCenter.x - 28, endpointCenter.y - 8, { steps: 2 });
...
test("selected edge target endpoints beat the overlapping edge body hit target", async ({ page }) => {
  ...
  await page.mouse.down();
  await expect(preview).toHaveCount(1);
  await page.mouse.move(endpointCenter.x - 14, endpointCenter.y, { steps: 2 });
...
test("selected edge reconnect previews and committed paths stay close to the horizontal lane", async ({ page }) => {
  ...
  const endpoint = page.getByRole("button", { name: "Reconnect target of edge edge-a-b" });
  const endpointBox = await endpoint.boundingBox();
  const targetHandle = page.getByRole("button", { name: "Connect into node 3" });
  ...
  await page.mouse.down();
  await expect(preview).toHaveCount(1);
  await page.mouse.move(probePoint.x, probePoint.y, { steps: 8 });
...
test("main editor can switch into and out of a large benchmark graph", async ({ page }) => {
  ...
  await expect(page.locator('[data-editor-perf="baseline-status"]')).toContainText("기준 상태: 수집 중");
  await expect(page.locator('[data-editor-perf="baseline-detail"]')).toContainText("F ");
  await expect(page.locator('[data-editor-perf="baseline-detail"]')).toContainText("W ");
...
test("main editor can reset perf instrumentation without resetting the graph", async ({ page }) => {
  ...
  await page.mouse.move(nodeTwoBox.x + nodeTwoBox.width / 2, nodeTwoBox.y + nodeTwoBox.height / 2);
  await page.mouse.down();
  await page.mouse.move(nodeTwoBox.x + nodeTwoBox.width / 2 + 48, nodeTwoBox.y + nodeTwoBox.height / 2 + 24, { steps: 8 });
  await page.mouse.up();

  await expect(page.locator('[data-editor-perf="samples"]')).not.toContainText("프레임: 0");
  await expect(page.locator('[data-editor-perf="avg-render"]')).not.toContainText("--");

  await page.getByRole("button", { name: "성능 계측 초기화" }).click();
...
test("@perf-live-capture benchmark live perf readout can be captured for harness-only evaluation", async ({ page }, testInfo) => {
  ...
  await page.addInitScript(() => {
    (window as Window & typeof globalThis & { __HF_CAPTURE_EDITOR_PERF__?: boolean }).__HF_CAPTURE_EDITOR_PERF__ = true;
  });
  ...
  await page.waitForFunction(
    (baseline) => {
      const captureWindow = window as Window & typeof globalThis & {
        __HF_EDITOR_PERF_GRAPH_PRESET__?: string;
        __HF_EDITOR_PERF_READOUT__?: {
      ...
      return (
        captureWindow.__HF_EDITOR_PERF_GRAPH_PRESET__ === "benchmark" &&
        captureWindow.__HF_EDITOR_PERF_READOUT__ &&
        (captureWindow.__HF_EDITOR_PERF_READOUT__.frameSampleCount ?? 0) >= baseline.minSamples &&
        (captureWindow.__HF_EDITOR_PERF_READOUT__.recentInteractionSampleCount ?? 0) >= baseline.recentInteractionWindow &&
        (captureWindow.__HF_EDITOR_PERF_READOUT__.interactionBurstCount ?? 0) >= baseline.minInteractionBursts
      );
    },
  ...
  const liveReadout = await page.evaluate(() => {
    const captureWindow = window as Window & typeof globalThis & {
      __HF_EDITOR_PERF_GRAPH_PRESET__?: string;
      __HF_EDITOR_PERF_READOUT__?: unknown;
    };
    return {
  ...
  await fs.mkdir(path.dirname(captureOutputPath), { recursive: true });
...
test("diagonally rightward edges keep using the right side instead of borrowing the top edge", async ({ page }) => {
  ...
  await page.mouse.down();
  await page.mouse.move(targetBox.x + targetBox.width / 2 + 140, targetBox.y + targetBox.height / 2 - 260, { steps: 12 });
  await page.mouse.up();

  const sourceBox = await sourceNode.boundingBox();
  const movedTargetBox = await targetNode.boundingBox();
  const editorRegionBox = await page.getByRole("region", { name: "HyperFlow 메인 editor" }).boundingBox();
...
test("dragging one selected node moves the rest of the selection with it", async ({ page }) => {
  ...
  await page.mouse.down();
  await page.mouse.move(beforeOne.x + beforeOne.width / 2 + 120, beforeOne.y + beforeOne.height / 2 + 48, { steps: 10 });
  await page.mouse.up();
