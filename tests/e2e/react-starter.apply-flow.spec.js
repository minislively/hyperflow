...
async function expectNodeHandlesVisibleAtRest(page, nodeId) {
    await expect(page.getByRole("button", {
        name: `Connect into node ${nodeId}`
    })).toBeVisible();
    await expect(page.getByRole("button", {
        name: `Connect from node ${nodeId}`
    })).toBeVisible();
}
...
test("main editor lets users add, drag, connect, and delete objects", async ({ page })=>{
    ...
    await page.mouse.down();
    await page.mouse.move((sourceFourHandleBox.x + sourceFourHandleBox.width / 2 + targetFiveHandleBox.x + targetFiveHandleBox.width / 2) / 2, (sourceFourHandleBox.y + sourceFourHandleBox.height / 2 + targetFiveHandleBox.y + targetFiveHandleBox.height / 2) / 2, {
        steps: 6
    });
...
test("selected edge target endpoints can be dragged to reconnect directly", async ({ page })=>{
    ...
    await page.mouse.down();
    await page.mouse.move(targetThreeHandleBox.x + targetThreeHandleBox.width / 2, targetThreeHandleBox.y + targetThreeHandleBox.height / 2, {
        steps: 12
    });
    await page.mouse.up();
    await page.getByRole("button", {
...
test("selected edge source endpoints can be dragged to reconnect directly", async ({ page })=>{
    ...
    const endpointBox = await endpoint.boundingBox();
    const sourceOneHandle = page.getByRole("button", {
    ...
    await page.mouse.move(endpointBox.x + endpointBox.width / 2, endpointBox.y + endpointBox.height / 2);
    await page.mouse.down();
    await page.mouse.move(sourceOneHandleBox.x + sourceOneHandleBox.width / 2, sourceOneHandleBox.y + sourceOneHandleBox.height / 2, {
        steps: 12
    });
    await page.mouse.up();
    await page.getByRole("button", {
...
test("dropping an edge endpoint reconnect on empty canvas keeps the original edge unchanged", async ({ page })=>{
    ...
    await page.mouse.move(endpointBox.x + endpointBox.width / 2, endpointBox.y + endpointBox.height / 2);
    await page.mouse.down();
    await page.mouse.move(canvasBox.x + canvasBox.width - 48, canvasBox.y + canvasBox.height - 48, {
        steps: 12
    });
    await page.mouse.up();
    await page.getByRole("button", {
    ...
    const snapshot = JSON.parse(snapshotText);
    const unchangedEdge = snapshot.edges.find((entry)=>Number(entry.source) === 1 && Number(entry.target) === 2);
