...
test("projectPocNodesToRuntimeNodes preserves order", ()=>{
    ...
    assert.deepEqual(runtimeNodes.map((node)=>node.id), [
        1,
        2
    ]);
...
test("createPocEdgeSpreadMaps fans out same-side siblings", ()=>{
    ...
    const { sourceSpreadByEdgeId } = createPocEdgeSpreadMaps(nodes.slice(), [
        {
            id: "e-1-2",
            source: 1,
            target: 2
...
test("resolvePocNodeRoleAnchorSide keeps diagonally rightward outputs on the right side", ()=>{
    ...
    const center = getPocNodeCenter(node);
    assert.equal(resolvePocNodeRoleAnchorSide(node, {
        toward: {
            x: center.x + 220,
            y: center.y - 240
        },
...
test("resolvePocEdgeCurveSpread centers slot-based offsets", ()=>{
    ...
    assert.equal(resolvePocEdgeCurveSpread({
        spread: 99,
        slot: 0,
        slotCount: 2,
        spreadStep: 18
...
test("resolvePocSmoothEdgeCurve returns cubic control points without moving endpoints", ()=>{
    const curve = resolvePocSmoothEdgeCurve(createPocEdgePathResolutionRequest({
        sourceAnchor: {
            x: 10,
            y: 20,
            side: "right",
