...
- handle을 눌러 edge 연결을 만들어 본다
- node를 추가하거나 현재 선택을 지워 본다
...
- add a node or delete the current selection
  ...
  const copy =
    locale === "ko"
      ? {
          ...
          body:
            mode === "basic-interactions"
              ? "주 설명은 위 문단이 담당한다. 아래 demo에서는 노드 드래그, 캔버스 pan, edge 연결, 노드 추가/삭제가 지금 어느 수준까지 되는지만 손으로 확인하면 된다."
      ...
      : {
          ...
          body:
            mode === "basic-interactions"
              ? "The main explanation lives in the text above. Use the demo below only to feel the current level of node drag, canvas pan, edge creation, and add/delete behavior."
