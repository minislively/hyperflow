...
test("evaluatePerfBaselineGate exposes warming evidence before thresholds are met", () => {
  const gate = evaluatePerfBaselineGate(
    createReadout({
      frameSampleCount: 40,
      interactionFrameSampleCount: 12,
      interactionBurstCount: 3,
...
test("evaluatePerfBaselineGate exposes machine-readable within-threshold evidence", () => {
  ...
  assert.deepEqual(
    gate.recentMetrics.map((metric) => metric.key),
    ["R", "V", "I", "P", "B"],
  );
  assert.deepEqual(
    gate.aggregateMetrics.map((metric) => metric.key),
    ["R", "V", "I", "P", "B"],
  );
});

test("comparePerfBaselineReadouts flags verdict changes before metric deltas", () => {
  const comparison = comparePerfBaselineReadouts(
    createReadout(),
    createReadout({
...
test("comparePerfBaselineReadoutEvidence exposes status-transition evidence before metric deltas", () => {
  const comparison = comparePerfBaselineReadoutEvidence(
    createReadout(),
    createReadout({
      recentAvgRenderMs: 16,
      recentAvgViewportMs: 9,
